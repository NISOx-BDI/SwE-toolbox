function varargout = swe_results_ui2(varargin)
% User interface for SwE/PPM results: Display and analysis of regional effects
% FORMAT [hReg,xSwE,SwE] = swe_results_ui2('Setup',[xSwE])
%
...
%
% The user is prompted to select a SwE{T} or SwE{F}, that is thresholded at
% user specified levels. The specification of the contrasts to use and the
% height and size thresholds are described in spm_getSwE.m. The resulting
% SwE is then displayed in the Graphics window as a maximum intensity
...
%                           ----------------
%
% In addition to setting up the results section, swe_results_ui2.m sets
...
% $Id: swe_results_ui2.m 7204 2017-11-08 17:26:52Z guillaume $
...
%
% swe_results_ui2 sets up and handles the SwE results graphical user
% interface, initialising an XYZ registry (see spm_XYZreg.m) to co-ordinate
% locations between various location controls.
%
%__________________________________________________________________________
%
% FORMAT [hreg,xSwE,SwE] = swe_results_ui2('Setup')
% Query SwE and setup GUI. 
%
% FORMAT [hreg,xSwE,SwE] = swe_results_ui2('Setup',xSwE)
% Query SwE and setup GUI using a xSwE input structure. This allows to run
% results setup without user interaction. See spm_getSwE for details of
% allowed fields.
%
% FORMAT hReg = swe_results_ui2('SetupGUI',M,DIM,xSwE,Finter)
...
%
% FORMAT swe_results_ui2('DrawButts',hReg,DIM,Finter,WS,FS)
% Draw GUI buttons
% hReg    - handle of XYZ registry object
% DIM     - 3 vector of image X, Y & Z dimensions
...
%
% FORMAT hFxyz = swe_results_ui2('DrawXYZgui',M,DIM,xSwE,xyz,hReg)
...
%
% FORMAT swe_results_ui2('EdWidCB')
% Callback for editable XYZ control widgets
%
% FORMAT swe_results_ui2('UpdateSwEval',hFxyz)
% FORMAT swe_results_ui2('UpdateSwEval',UD)
...
%
% FORMAT xyz = swe_results_ui2('GetCoords',hFxyz)
...
%
% FORMAT [xyz,d] = swe_results_ui2('SetCoords',xyz,hFxyz,hC)
...
% NB: When using the results section, should use XYZregistry to get/set location
%
% FORMAT hFxyz = swe_results_ui2('FindXYZframe',h)
...
%
% FORMAT hMP = swe_results_ui2('LaunchMP',M,DIM,hReg,hBmp)
% Prototype callback handler for integrating MultiPlanar toolbox
%
% FORMAT swe_results_ui2('Delete',h)
...
%==========================================================================
    ...
    hReg      = swe_results_ui2('SetupGUI',M,DIM,xSwE,Finter);
    ...
    %-Setup contrast menu
    %----------------------------------------------------------------------
    hConUI    = swe_results_ui2('SetupConMenu',xSwE,SwE,Finter);
    
    %-Atlas menu
    %----------------------------------------------------------------------
    if isequal(units,{'mm' 'mm' 'mm'})
        hAtlasUI = swe_results_ui2('SetupAtlasMenu',Finter);
    end
    ...
    text(0.5,0.5,xSwE.title,'Parent',hTitAx,...
    ...
    h     = text(0,24,'SwEresults:','Parent',hResAx,...
        'FontWeight','Bold','FontSize',FS(14));
    text(get(h,'Extent')*[0;0;1;0],24,spm_file(SwE.swd,'short30'),'Parent',hResAx)
    try
        thresDesc = xSwE.thresDesc;
        if strcmp(xSwE.STAT,'P')
            text(0,12,sprintf('Height threshold %s',thresDesc),'Parent',hResAx)
        else
            text(0,12,sprintf('Height threshold %c = %0.6f  {%s}',xSwE.STAT,xSwE.u,thresDesc),'Parent',hResAx)
        end
    catch
        text(0,12,sprintf('Height threshold %c = %0.6f',xSwE.STAT,xSwE.u),'Parent',hResAx)
    end
    if spm_mesh_detect(xSwE.Vspm), str = 'vertices'; else str = 'voxels'; end
    text(0,00,sprintf('Extent threshold k = %0.0f %s',xSwE.k,str), 'Parent',hResAx)
 
 
    %-Plot design matrix
    %----------------------------------------------------------------------
    hDesMtx   = axes('Parent',Fgraph,'Position',[0.65 0.55 0.25 0.25]);
    hDesMtxIm = image((SwE.xX.X + 1)*32,'Parent',hDesMtx);
    ...
    for ii = nCon:-1:1
        ...
        ylabel(hCon,num2str(xSwE.Ic(ii)),'FontSize',FS(10),'FontWeight','normal')
    ...
    %======================================================================
        ...
        %------------------------------------------------------------------
        hFxyz      = swe_results_ui2('DrawXYZgui',M,DIM,varargin{4},xyz,hReg);
        spm_XYZreg('XReg',hReg,hFxyz,'swe_results_ui2');
 
        %-Set up buttons for results functions
        %------------------------------------------------------------------
        swe_results_ui2('DrawButts',hReg,DIM,Finter,WS,FS);
    ...
    %======================================================================
        ...
        uicontrol('Parent',hPan,'Style','PushButton','String','small volume',...
            ...
            'Callback','TabDat = spm_VOI(SwE,xSwE,hReg);',...
            'Interruptible','on','Enable','on',...
        ...
        uicontrol('Parent',hPan,'Style','PushButton','String','CVA',...
            ...
            'Callback','CVA = spm_cva_ui('''',xSwE,SwE)',...
            'Interruptible','on','Enable','on',...
            'FontSize',FS(10));
        uicontrol('Parent',hPan,'Style','PushButton','String','multivariate Bayes',...
            ...
            'ToolTipString',...
            'Multivariate Bayes',...
            'Callback','[MVB] = spm_mvb_ui(xSwE,SwE)',...
            'Interruptible','on','Enable','on',...
            'FontSize',FS(10));
        ...
        uicontrol('Parent',hPan,'Style','PushButton','String','plot',...
            ...
            'Callback','[Y,y,beta,Bcov] = spm_graph_ui(xSwE,SwE,hReg);',...
            'Interruptible','on','Enable','on',...
            'Position',[005 055 100 020].*WS,...
            'Tag','plotButton');
        ...
        uicontrol('Parent',hReg,'Style','PushButton','String','clear',...
            ...
            'Callback',['swe_results_ui2(''Clear''); ',...
        ...
        uicontrol('Parent',hReg,'Style','PushButton','String','exit',...
            ...
            'Callback','swe_results_ui2(''close'')',...
            'Interruptible','on','Enable','on',...
    ...
    %======================================================================
    % swe_results_ui2('SetupConMenu',xSwE,SwE,Finter)
    ...
    if strcmp(xSwE.STAT,'P')
        xSwEtmp = xSwE; xSwEtmp.thresDesc = 'LogBF';
        uimenu(hC1,'Label','Set LogBF','UserData',struct('Ic',xSwE.Ic),...
            'Callback',{@mychgcon,xSwEtmp});
    else
        xSwEtmp = xSwE; xSwEtmp.thresDesc = 'p<0.05 (FWE)';
    ...
    %======================================================================
        % swe_results_ui2('SetupAtlasMenu',Finter)
        ...
        for i=1:numel(list)
            uimenu(hC1,'Label',list(i).name,...
                'Callback',sprintf('swe_list(''label'',''%s'');',list(i).name));
        end
    ...
    %======================================================================
        % hFxyz = swe_results_ui2('DrawXYZgui',M,DIM,xSwE,xyz,hReg)
        ...
        hX = uicontrol('Parent',hReg,'Style','Edit','String',sprintf('%.2f',xyz(1)),...
            ...
            'Tag','hX',...
            'Callback','swe_results_ui2(''EdWidCB'')');
        ...
        hPan = uipanel('Parent',hReg,'Title','statistic','Units','Pixels',...
            ...
            'ForegroundColor',[1 1 1],...
            'BackgroundColor',[179 179 179]/255);
        hSwE = uicontrol('Parent',hPan,'Style','Text','String','',...
    ...
    %======================================================================
        ...
        if abs(xyz(d)-nxyz(d))>0
            ...
            swe_results_ui2('UpdateSwEval',UD)
        end
 
        
    %======================================================================
    case 'updatesweval'                           %-Update SwE value in GUI
    %======================================================================
        % swe_results_ui2('UpdateSwEval',hFxyz)
        % swe_results_ui2('UpdateSwEval',UD)
    ...
    %======================================================================
        % [xyz,d] = swe_results_ui2('SetCoords',xyz,hFxyz,hC)
        if nargin<4, hC=0; else hC=varargin{4}; end
        if nargin<3, hFxyz=swe_results_ui2('FindXYZframe'); else hFxyz=varargin{3}; end
    ...
    %======================================================================
        % hFxyz = swe_results_ui2('FindXYZframe',h)
    ...
    %======================================================================
        ...
        h4 = uicontrol('Parent',hGraphUIButtsF,'Style','popupmenu',...
            ...
            'Callback','swe_results_ui2(''PlotUiCB'')',...
            'Interruptible','on','Enable','on',...
        ...
        set(hGraphUIbg,'UserData',...
            ...
            'DeleteFcn','swe_results_ui2(''Delete'',get(gcbo,''UserData''))')
    ...
    %======================================================================
        % Fgraph = swe_results_ui2('Clear',F,mode)
    ...
    %======================================================================
        % hMP = swe_results_ui2('LaunchMP',M,DIM,hReg,hBmp)
        ...
        set(hBmp,'DeleteFcn','swe_results_ui2(''delete'',get(gcbo,''UserData''))')
...
if ~isempty(xSwE.thresDesc)
    if strcmp(xSwE.STAT,'P')
        % These are soon overwritten by spm_getSwE
        xSwE2.thresDesc = xSwE.thresDesc;
        xSwE2.u = xSwE.u;
        xSwE2.k = xSwE.k;
        % xSwE.STATstr contains Gamma
    else
...
[hReg,xSwE,SwE] = swe_results_ui2('setup',xSwE2);
...
%==========================================================================
function mycheckres(obj,evt,xSwE)
%==========================================================================
spm_check_results([],xSwE);
...
switch lower(action)
    ...
    case 'current'
        [xyzmm,i] = spm_XYZreg('NearestXYZ',...
            swe_results_ui2('GetCoords'),xSwE.XYZmm);
        swe_results_ui2('SetCoords',xSwE.XYZmm(:,i));
...
if isfield(xSwE,'G')
    ...
    sprintf('SwE{%c}-filtered: u = %5.3f, k = %d',xSwE.STAT,xSwE.u,xSwE.k));
