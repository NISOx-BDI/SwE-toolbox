...
%--------------------------------------------------------------------------
if exist(fullfile(SwE.swd,'swe_vox_mask.img'),'file') == 2
...
if ~isMat
  ...
  %----------------------------------------------------------------------
  VM    = struct('fname',  'swe_vox_mask.img',...
    'dim',    DIM',...
  ...
  Vscore = swe_create_vol(['swe_vox_' WB.stat 'stat_c0001.img'], DIM, M,...
			  ['Original parametric ' WB.stat ' statistic data.']);
  
  %-Initialise parametric P-Value image
  %----------------------------------------------------------------------
  
  VlP = swe_create_vol(['swe_vox_' WB.stat 'stat_lp_c0001.img'], DIM, M,...
                      'Original parametric -log10(P) value data (positive).');
  
  if WB.stat=='T'
        VlP_Neg = swe_create_vol(['swe_vox_' WB.stat 'stat_lp_c0001neg.img'], DIM, M,...
  ...
  VcScore = swe_create_vol(['swe_vox_' OutStat WB.stat 'stat_c0001.img'], DIM, M,...
  ...
  %----------------------------------------------------------------------
  VlP_wb_pos = swe_create_vol(['swe_vox_' WB.stat 'stat_lp-WB_c0001.img'], DIM, M,...
                              '-log10(uncor. non-para. P, +ve)');

  VlP_wb_FWE_pos = swe_create_vol(['swe_vox_' WB.stat 'stat_lpFWE-WB_c0001.img'], DIM, M,...
                                  '-log10(FWE-corr. P, +ve)');
  
  VlP_wb_FDR_pos = swe_create_vol(['swe_vox_' WB.stat 'stat_lpFDR-WB_c0001.img'], DIM, M,...
                                  '-log10(FDR-corr. P, +ve)');

  if WB.stat=='T'
    VlP_wb_neg = swe_create_vol(['swe_vox_' WB.stat 'stat_lp-WB_c0001neg.img'], DIM, M,...
                              '-log10(uncor. non-para. P, -ve)');
    
    VlP_wb_FWE_neg = swe_create_vol(['swe_vox_' WB.stat 'stat_lpFWE-WB_c0001neg.img'], DIM, M,...
                                     '-log10(FWE-corr. P, -ve)');
    
    VlP_wb_FDR_neg = swe_create_vol(['swe_vox_' WB.stat 'stat_lpFDR-WB_c0001neg.img'], DIM, M,...
                                     '-log10(FDR-corr. P, -ve)');
    
    % In this case the Vscore images are Z images.
    Vscore_wb_pos = swe_create_vol(['swe_vox_zTstat-WB_c0001.img'], DIM, M,...
  ...
  if WB.stat=='F'
    ...
    Vscore_wb_pos = swe_create_vol(['swe_vox_xFstat-WB_c0001.img'], DIM, M,...
  ...
  if WB.clusterWise == 1
    ...
    VlP_wb_clusterFWE_pos = swe_create_vol(['swe_clustere_' WB.stat 'stat_lpFWE-WB_c0001.img'], DIM, M,...
                                           '-log10(clusterFWE-corr. P, +ve)');
    
    if WB.stat=='T'
      VlP_wb_clusterFWE_neg = swe_create_vol(['swe_clustere_' WB.stat 'stat_lpFWE-WB_c0001neg.img'], DIM, M,...
                                             '-log10(clusterFWE-corr. P, -ve)');
    end
  end
