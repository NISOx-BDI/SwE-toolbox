...
function setup()
    ...
    zipped_data= websave(fullfile(fileparts(mfilename('fullpath')),...
                            'data'),...
                       ['http://www.fil.ion.ucl.ac.uk/spm/download/data'...
    ...
    websave(fullfile(unzipped_data, '..', 'data', 'mat_input',...
                     'subj_data.mat'),...
            ['https://drive.google.com/uc?export=download&id=1RXHFtnB1'...
             'N14-FcOuda8139zgR5dnjcly']);
    websave(fullfile(unzipped_data, '..', 'data', 'mat_input',...
                 'tri_data.mat'),...
        ['https://drive.google.com/uc?export=download&id=1vuHGkPvQMulp'...
