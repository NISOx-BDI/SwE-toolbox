...
if ~isMat
    ...
    %----------------------------------------------------------------------
    disp(file_ext)
    VM    = swe_create_vol(['swe_vox_mask' file_ext], DIM, M,...
    ...
    for i = 1:nBeta
        Vbeta(i) = swe_create_vol([sprintf('swe_vox_beta_b%04d',i) file_ext],...
                                  DIM, M,...
    ...
    for i=1:nBeta
        for ii=i:nBeta
            it=it+1;
            Vcov_beta(it) = swe_create_vol([sprintf('swe_vox_cov_b%04d_b%04d',i,ii) file_ext],...
    ...
    if dof_type == 1
        ...
        for g=1:nGr
            for ii=1:nBeta
                for iii=ii:nBeta
                    it=it+1;
                    Vcov_beta_g(it) = swe_create_vol([sprintf('swe_vox_cov_g%04d_b%04d_b%04d',g,ii,iii) file_ext],...
    ...
    if isfield(SwE.type,'modified')
        ...
        for g =1:nGr
            for ii=1:nVis_g(g)
                for iii=ii:nVis_g(g)
                    it=it+1;
                    Vcov_vis(it) = swe_create_vol([sprintf('swe_vox_cov_g%04d_v%04d_v%04d',g,ii,iii) file_ext],...
