    ...
    if isempty(xCon(ic).Vspm)
        ...
        if nSizeCon==1
            ...
            if isMat
              ...
              %------------------------------------------------------
              xCon(ic).Vcon = sprintf('swe_vox_con_c%02d%s',ic,file_ext);
...
%                           Z2(Z>0) = swe_invNcdf(0.5 - spm_Fcdf(Z,xCon(ic).eidf,xCon(ic).edf-xCon(ic).eidf+1)/2).^2;
        ...
        if isMat
          ...
          xCon(ic).Vspm2 = struct(...
            'fname',  sprintf('swe_vox_%cstat_c%02d%s',xCon(ic).STAT,ic,file_ext),...
            'dim',    SwE.xVol.DIM',...
            'dt',     [spm_type('float32'), spm_platform('bigend')],...
            'mat',    SwE.xVol.M,...
            'pinfo',  [1,0,0]',...
        ...
        if isMat
          ...
          xCon(ic).VspmUncP = struct(...
            'fname',  sprintf('swe_vox_%cstat_lp_c%02d%s',xCon(ic).STAT,ic,file_ext),...
            'dim',    SwE.xVol.DIM',...
            'dt',     [spm_type('float32'), spm_platform('bigend')],...
            'mat',    SwE.xVol.M,...
            'pinfo',  [1,0,0]',...
        ...
        if dof_type
          if isMat
            ...
            xCon(ic).Vedf = struct(...
              'fname',  sprintf('swe_vox_edf_c%02d%s',ic,file_ext),...
              'dim',    SwE.xVol.DIM',...
              'dt',     [spm_type('float32'), spm_platform('bigend')],...
              'mat',    SwE.xVol.M,...
              'pinfo',  [1,0,0]',...
