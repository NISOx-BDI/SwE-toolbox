...
% Fills in SwE.xCon and writes con_????.*, ess_????.* and SwE?_????.*
...
% Going forward, use default file extension (for ~isMat)
    ...
    if isempty(xCon(ic).Vspm)
        ...
        if nSizeCon==1
            ...
            if isMat
              ...
              %------------------------------------------------------
              xCon(ic).Vcon = struct(...
                'fname',  sprintf(['con_%04d' file_ext],ic),...
                'dim',    SwE.xVol.DIM',...
...
%                           Z2(Z>0) = swe_invNcdf(0.5 - spm_Fcdf(Z,xCon(ic).eidf,xCon(ic).edf-xCon(ic).eidf+1)/2).^2;
        ...
        if isMat
          ...
          xCon(ic).Vspm = struct(...
            'fname',  sprintf(['spm%c_%04d' file_ext],eSTAT,ic),...
            'dim',    SwE.xVol.DIM',...
            'dt',     [spm_type('float32'), spm_platform('bigend')],...
            'mat',    SwE.xVol.M,...
            'pinfo',  [1,0,0]',...
        ...
        if isMat
          ...
          xCon(ic).Vspm2 = struct(...
            'fname',  sprintf(['spm%c_%04d' file_ext],xCon(ic).STAT,ic),...
            'dim',    SwE.xVol.DIM',...
            'dt',     [spm_type('float32'), spm_platform('bigend')],...
            'mat',    SwE.xVol.M,...
            'pinfo',  [1,0,0]',...
        ...
        if isMat
          ...
          xCon(ic).VspmUncP = struct(...
            'fname',  sprintf(['spm%s_%04d' file_ext],'UncP',ic),...
            'dim',    SwE.xVol.DIM',...
            'dt',     [spm_type('float32'), spm_platform('bigend')],...
            'mat',    SwE.xVol.M,...
            'pinfo',  [1,0,0]',...
        ...
        if dof_type
          if isMat
            ...
            xCon(ic).Vedf = struct(...
              'fname',  sprintf(['edf_%04d' file_ext],ic),...
              'dim',    SwE.xVol.DIM',...
